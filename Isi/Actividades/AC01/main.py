...
def cargar_emociones(ruta_archivo_emociones):
    ...
    """
    archivo_emociones = open("emociones.txt")
    set_emociones = set()
    lista_emociones=[]
    for linea in archivo_emociones:
        nueva_linea = linea.strip()
        id, emocion = nueva_linea.split(",")
        tupla = (id, emocion)
        set_emociones.add(tupla)
    archivo_emociones.close()
    return set_emociones

def cargar_cims(ruta_archivo_cims, ruta_archivo_emociones):
    """
    Esta función carga los cims y los retorna en una estructura.
    """
    archivo_cims=open("cims.txt")
    registro_cims=namedtuple("Cims",[])
    archivo_cims=open("cims.txt")
    registro_cims=namedtuple("Cims",["nombre","edad","actividades"])
    cim=""
    diccionario_cims={}
    for linea in archivo_cims:
        datos=linea.split(",")
        actividad=[]
        for i in range (3,len(datos)):
            act=datos[i]
            actividad.append(act)
        cola_acciones=deque(actividad)
        id=datos[0]
        nombre=datos[1]
        edad=datos[2]
        cim=(registro_cims(nombre,edad,actividades))
        diccionario_cims[id]=cim
